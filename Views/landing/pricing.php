...
$buttonFree = ($usernameArray) ? 'Proceed to API Keys' : 'Get Started Now';
$buttonLink = ($usernameArray) ? '/api-keys' : '/login?destination=/api-keys';
$pricingPlans = [
    'Free' => [
...
foreach ($pricingPlans as $title => $pricingArrays) {
    echo '<div class="p-4">';
        echo '<div class="flex flex-col pt-8 pb-8 h-full bg-gray-100 hover:bg-' . $theme . '-300 dark:bg-gray-900 dark:hover:bg-gray-700 rounded-md shadow-md hover:scale-105 transition duration-500">';
            echo '<div class="px-8">';
                echo Html::h2($title);
    echo '<p class="mb-6 text-coolGray-400 font-medium">' . $pricingArrays['description'] . '</p>';
    echo '<div class="mb-6">';
    echo '<span class="relative -top-10 right-1 text-3xl text-coolGray-900 font-bold">$</span>';
    echo '<span class="text-6xl md:text-7xl text-coolGray-900 font-semibold tracking-tighter">' . $pricingArrays['cost'] . '</span>';
    echo '<span class="inline-block ml-1 text-coolGray-500 font-semibold">/mo</span>';
    echo '</div>';
    echo '<a class="inline-block py-4 px-7 mb-8 w-full text-base md:text-lg leading-6 text-' . $theme . '-50 font-medium text-center bg-' . $theme . '-500 hover:bg-' . $theme . '-600 focus:ring-2 focus:ring-' . $theme . '-500 focus:ring-opacity-50 rounded-md shadow-sm" href="' . $pricingArrays['buttonLink'] . '">' . $pricingArrays['button'] . '</a>';
    echo '<ul class="self-start px-8">';
    foreach ($pricingArrays['features'] as $feature) {
